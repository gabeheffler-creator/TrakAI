...
interface AlarmApp {
  ...
  emoji: string;
  platform: string;
  appUrl?: string; // URL scheme used to open the app directly
}
...
const ALARM_APPS: AlarmApp[] = [
  ...
  },
  {
    ...
    emoji: "🕐",
    platform: "Android",
    appUrl: "https://assistant.google.com/",
  },
  {
    id: "bedtime",
...
function AlarmSheet({
  ...
  const handleConfirm = () => {
    ...
    // Open the alarm app via URL scheme if available
    if (confirmApp.appUrl) {
      window.open(confirmApp.appUrl, "_blank");
    }
    // Save connection immediately
    onConnect(confirmApp.id);
    setConnectedView(confirmApp);
    setConfirmApp(null);
  };

  const handleCancel = () => setConfirmApp(null);
  ...
  const handleOpenApp = (app: AlarmApp) => {
  ...
  return (
    <>
          ...
          <SheetHeader className="mb-4">
            {connectedView ? (
              <div className="flex items-center gap-2">
                ...
                <SheetTitle>{connectedView.emoji} {connectedView.name} connected</SheetTitle>
              </div>
            ) : (
          ...
          {connectedView ? (
            ...
            <div className="pb-4">
              ...
              <div className="flex flex-col gap-2 mt-2">
                ...
                <Button variant="ghost" className="w-full text-muted-foreground" onClick={handleChangeApp}>
                  Change alarm app
                </Button>
              </div>
            </div>
      ...
      {/* Confirmation dialog — appears over the sheet */}
      <AlertDialog open={!!confirmApp} onOpenChange={v => !v && handleCancel()}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {confirmApp?.emoji} Connect {confirmApp?.name}?
            </AlertDialogTitle>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleConfirm}>Connect alarm</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
...
export function SleepPage() {
  ...
  const handleAlarmConnect = (appId: string) => {
    ...
    if (clientId) {
      updateClient.mutate(
        { clientId, data: { connectedAlarmAppId: appId } },
        { onSuccess: () => qc.invalidateQueries({ queryKey: getGetMyClientQueryKey() }) },
      );
    }
  };

  const handleAlarmDisconnect = () => {
    localStorage.removeItem(ALARM_STORAGE_KEY);
    if (clientId) {
      updateClient.mutate(
        { clientId, data: { connectedAlarmAppId: null } },
        { onSuccess: () => qc.invalidateQueries({ queryKey: getGetMyClientQueryKey() }) },
      );
    }
      ...
      {connectedApp ? (
        <div className="flex gap-2">
          ...
          <Button
            ...
            data-testid="button-disconnect-alarm"
          >
            Disconnect
          </Button>
        </div>
      ...
      <AlertDialog open={disconnectDialogOpen} onOpenChange={setDisconnectDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will disconnect {connectedApp?.name} from your sleep tracker.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
