...
function isMobileFrame() {
  ...
  if (new URLSearchParams(window.location.search).get("mobile") === "1") {
    sessionStorage.setItem("trak_mobile_frame", "1");
    return true;
  }
  return sessionStorage.getItem("trak_mobile_frame") === "1";
}
              ...
              <nav className="flex-1 px-3 space-y-1 overflow-y-auto">
                {navigation.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <item.icon className={cn("h-4 w-4 mr-3 flex-shrink-0", isActive ? "text-primary-foreground" : "text-muted-foreground")} />
                      {item.name}
