...
const storagePrefix = "com.trakai.coach.";
...
async function clearTokens() {
  accessToken = null; refreshToken = null; await SecureStorage.remove("refresh-token");
}
async function clearPushToken() {
  currentDeviceToken = null; registeredPushToken = null; await SecureStorage.remove("push-token");
}
async function performRefresh() {
  if (!refreshToken) return false;
  const response = await originalFetch(apiUrl("/api/auth/token/refresh"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ refreshToken }) });
  if (!response.ok) { await clearTokens(); return false; }
  await persist(await response.json()); return true;
}
function refresh() {
  if (!refreshPromise) {
...
async function nativeFetch(input: RequestInfo | URL, init: RequestInit = {}) {
  ...
  if (path === `/api/auth/${role}/login`) {
    ...
    body = JSON.stringify({ ...credentials, role, deviceLabel: "TrakAI iOS" }); headers.set("Content-Type", "application/json");
  } else if (path === "/api/auth/logout") {
    await unregisterPushToken().catch(() => undefined);
    url = apiUrl("/api/auth/token/revoke"); body = JSON.stringify({ refreshToken, deviceToken: registeredPushToken }); headers.set("Content-Type", "application/json");
  }
  ...
  return request().then(async response => {
    ...
    headers.set("X-Trak-Retry", "1"); return (await refresh()) ? request() : response;
  });
}
function route(url: string) {
  const link = new URL(url, window.location.origin);
  let path = link.pathname;
  if (link.protocol === "trakai-coach:" && link.host) path = `/${link.host}${path === "/" ? "" : path}`;
...
async function registerPushToken(token: string) {
  ...
  await window.fetch("/api/push-tokens", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ deviceToken: token }) });
  registeredPushToken = token;
}
async function unregisterPushToken() {
  const token = currentDeviceToken ?? registeredPushToken;
  if (!token) return;
  const response = await window.fetch("/api/push-tokens", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ deviceToken: token }) });
