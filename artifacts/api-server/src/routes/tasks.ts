...
// PATCH /api/clients/:clientId/tasks/:taskId/resubmit — client resubmits a rejected task
router.patch("/clients/:clientId/tasks/:taskId/resubmit", requireClientOwnership(), async (req, res) => {
  try {
    const clientId = Number(req.params.clientId);
    const taskId = Number(req.params.taskId);
    const actor = req.actor;
    ...
    const task = await verifyClientOwnsTask(clientId, taskId);
    if (!task) { res.status(404).json({ error: "Task not found" }); return; }
    if (task.status !== "rejected") { res.status(409).json({ error: "Task is not rejected" }); return; }
    ...
    const [updated] = await db.update(clientTasksTable).set({
      ...
      updatedAt: new Date(),
    }).where(and(eq(clientTasksTable.id, taskId), eq(clientTasksTable.clientId, clientId))).returning();

    const messageContent = body.note?.trim()
      ? `Resubmitting task: ${body.note.trim()}`
      : "I'd like to try this task again ↩️";
    ...
    req.log.error(err);
    res.status(400).json({ error: "Failed to resubmit task" });
  }
});
